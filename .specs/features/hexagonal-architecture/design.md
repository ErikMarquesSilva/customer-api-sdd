...
`TransactionBoundaryIntegrationTest` checks the transaction behaviourally. It observes an active read-write transaction at the pre-check of POST and PUT, and catches method-level overrides (X5) that a class-level rule cannot see. The same test asserts, using Hibernate statistics, that each POST and PUT completes exactly one transaction. This proves the read and the versioned write are not split across transactions (verifier mutants Y7/Y8, closed by T16 after the user chose option b). The CUST-24 HTTP test also asserts the invariant itself: Hibernate records exactly one optimistic failure, so the stale write reached the versioned UPDATE. This also covers an adapter that discards the entity it already read (Z7b/Z7c, T17).
...
| ------- | -------- | ------ | ---------- |
| CUST-24 (optimistic lock) could silently turn into a lost update if the adapter reloads a fresh entity | `CustomerPersistenceAdapter.update` | Data loss | Two defenses (corrected after verification). **In production** the use case runs in one transaction (an ArchUnit rule requires class-level `@Transactional` with a propagation that starts a transaction: REQUIRED, REQUIRES_NEW or NESTED), Hibernate's optimistic check runs against the version the service read, and the versioned UPDATE plus the `ObjectOptimisticLockingFailureException` translation catch the conflict. The CUST-24 HTTP test covers this, including the assertion that exactly one optimistic failure occurred. The explicit version comparison only fires for callers outside a transaction (covered by the adapter test). With neither defense, a lost update occurs, which is why the transaction rule exists |
...
| CUST-24 test pins the detection mechanism | The test asserts exactly one Hibernate optimistic failure as well as the 409 and the kept data | Every route to a lost update was caught in verification. Trade-off: a deliberate change of mechanism (e.g. a service-level version re-check) must update this test and this design together |
